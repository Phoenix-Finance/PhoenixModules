...
contract acceleratedMinePool is acceleratedMinePoolData {
    ...
    function setBoostingNFTAddress(address _nftAddress) external onlyOwner {
    ...
    function changePPTBalance(address account) internal settleDistribution(account) {
    ...
    function changeAcceleratedInfo(address account,uint256[] memory newRates,uint256 maxLockedPeriod) public onlyAccelerator settleDistribution(account){
...
//        userInfoMap[account].maxPeriodID =  0;
    ...
    function getPeriodWeight(address account,uint256 currentID,uint256 maxPeriod) public view returns (uint256) {
        uint256 tokenID = userNFTDeposit[account];
        uint256 nftBoosting = rateDecimal;
        if (tokenID != 0x0) {
            nftBoosting = IZooNFT(NFTAddress).getBoosting(tokenID)*rateDecimal/1e12;
        }
        ...
        if(userInfoMap[account].periodRates.length > curLocked){
            return userInfoMap[account].periodRates[curLocked] + nftBoosting - rateDecimal;
        }
        return nftBoosting;
    }
    modifier settleDistribution(address account){
    ...
    function depositNFT(uint _tokenId) external settleDistribution(msg.sender) {
        uint256 tokenID = userNFTDeposit[msg.sender];
        if (_tokenId != 0x0) {
            if (tokenID != 0x0) {
                IZooNFT(NFTAddress).safeTransferFrom(address(this), msg.sender, tokenID);
                emit BoostingWithdraw(msg.sender, tokenID);
            }
            userNFTDeposit[msg.sender] = _tokenId;
            IZooNFT(NFTAddress).safeTransferFrom(msg.sender, address(this), _tokenId);
        }
        emit BoostingDeposit( msg.sender, _tokenId);
    }
    function withdrawNFT() external settleDistribution(msg.sender) {
        uint256 tokenID = userNFTDeposit[msg.sender];
        require(tokenID!=0, "Boosting NFT token is not deposited.");
        IZooNFT(NFTAddress).safeTransferFrom(address(this), msg.sender, tokenID);
        tokenID = 0x0;
